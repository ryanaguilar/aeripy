...
class AeripyApi:
    ...
    def get_system_info(self) -> SystemInfo:
        ...
        sys_info = SystemInfo(**util.snake_case_keys(result.data))
        return sys_info

    def get_schools(self) -> List[School]:
        result = self._rest_adapter.get(endpoint=API_PATH['schools'])
        schools_list = [School(**util.snake_case_keys(datum)) for datum in result.data]
        return schools_list
    ...
    def get_school(self, school_code: int) -> School:
        ...
        school = School(**util.snake_case_keys(result.data))
        return school
    ...
    def get_terms(self, school_code: int) -> List[Term]:
        ...
        terms_list = [Term(**util.snake_case_keys(datum)) for datum in result.data]
        return terms_list
    ...
    def get_bell_schedules(self, school_code: int, date: int=None) -> List[BellScheduleElement]:
        ...
        bell_schedules = [BellScheduleElement(**util.snake_case_keys(datum)) for datum in result.data]
    ...
    def get_bell_schedule(self, school_code: int, date: str) -> BellScheduleElement:
        ...
        bell_schedule = BellScheduleElement(**util.snake_case_keys(result.data))
    ...
    def get_calendar(self, school_code: int) -> List[CalendarElement]:
        ...
        calendar = [CalendarElement(**util.snake_case_keys(datum)) for datum in result.data]
        return calendar
    ...
    def get_absence_codes(self, school_code: int, absence_code: int = None) -> List[AbsenceCodeElement]:
        ...
        result = self._rest_adapter.get(endpoint=endpoint)
        absence_codes_list = [AbsenceCodeElement(**util.snake_case_keys(datum)) for datum in result.data]
    ...
    def get_absence_code(self, school_code: int, absence_code: int) -> AbsenceCodeElement:
        ...
        absence_code = AbsenceCodeElement(*util.snake_case_keys(result.data))
        return absence_code
    ...
    def get_staff(self, staff_id: int = None) -> List[StaffElement]:
        ...
        result = self._rest_adapter.get(endpoint=endpoint)
        staff_list = [StaffElement(**util.snake_case_keys(datum)) for datum in result.data]
        return staff_list

    def insert_staff(self, data: dict) -> StaffElement:
        result = self._rest_adapter.post(endpoint=API_PATH["staff"], data=util.camel_case_keys(data))
        staff = StaffElement(**util.snake_case_keys(result.data))
        return staff

    def update_staff(self, data: dict, staff_id: int = None) -> StaffElement:
        if staff_id is None:
            staff_id = data.get("staff_id")
        result = self._rest_adapter.put(endpoint=API_PATH["staff_id"].format(staff_id=staff_id), data=util.camel_case_keys(data))
        staff = StaffElement(**util.snake_case_keys(result.data))
        return staff
